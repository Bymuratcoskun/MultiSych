...
namespace MultiSych.Services.Implementations
{
    public class CloudCalendarService : ICalendarService
    {
        ...
        public CloudCalendarService(IHttpClientFactory httpClientFactory, IDbContextFactory<LocalCacheDbContext> dbContextFactory)
        {
        ...
        {
            _logger.Information("Starting cloud calendar sync for {Email}...", credentials.Email);
            try
            {
                // Örneğin geçmiş 30 gün ile gelecek 90 günlük etkinlikleri çekiyoruz
                var startDate = DateTime.UtcNow.AddDays(-30);
                var endDate = DateTime.UtcNow.AddDays(90);
                var events = await GetEventsAsync(credentials, startDate, endDate);

                await using var dbContext = await _dbContextFactory.CreateDbContextAsync();
                foreach (var ev in events)
                {
                    var existing = await dbContext.CachedEvents.FindAsync(ev.AccountId, ev.EventId);
                    if (existing != null)
                    {
                        existing.Title = ev.Title ?? string.Empty;
                        existing.Description = ev.Description ?? string.Empty;
                    ...
                    {
                        await dbContext.CachedEvents.AddAsync(new CalendarEventEntity
                        {
                            AccountId = ev.AccountId ?? string.Empty,
                            EventId = ev.EventId ?? string.Empty,
                            Title = ev.Title ?? string.Empty,
                            Description = ev.Description ?? string.Empty,
                            StartTime = ev.StartTime,
                            EndTime = ev.EndTime,
                            Provider = ev.Provider ?? string.Empty,
                ...
                _logger.Information("Successfully synced {Count} events for {Email}", events.Count, credentials.Email);
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Failed to sync calendar events for {Email}", credentials.Email);
            }
        }

        // --- Google Calendar Implementations ---

        private Google.Apis.Calendar.v3.CalendarService GetGoogleCalendarService(AccountCredentials credentials)
        {

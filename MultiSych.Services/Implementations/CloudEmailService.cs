...
namespace MultiSych.Services.Implementations
{
    public class CloudEmailService : IEmailService
    {
        ...
        public CloudEmailService(IHttpClientFactory httpClientFactory, IDbContextFactory<LocalCacheDbContext> dbContextFactory)
        {
        ...
        {
            _logger.Information("Starting cloud email sync for {Email}...", credentials.Email);
            try
            {
                ...
                _logger.Information("Successfully synced {Count} emails for {Email}", emails.Count, credentials.Email);
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Failed to sync emails for {Email}", credentials.Email);
            }
        }
    }
}
